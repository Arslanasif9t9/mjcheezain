...
- **⚠ DEPLOY NOTE**: autodeploy FTP-only hai — `category_suggestions` table server DB par manually banani hogi (ya `php artisan migrate --path=database/migrations/2026_07_16_000001_create_category_suggestions_table.php` server par chalana hoga) warna Other-category flow live par error dega (suggestion insert try/catch mein hai — product create phir bhi hoga, sirf suggestion log-warning banegi).
