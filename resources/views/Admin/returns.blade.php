    ...
    <main class="flex-1 min-w-0 p-6 lg:p-8">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Customer return &amp; refund requests — update status inline.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 mb-6">
            @php
                $cards = [
                    ...
                    ['label' => 'Refunded', 'value' => $stats['refunded'], 'icon' => 'fa-hand-holding-dollar', 'cls' => 'bg-green-100 text-green-600'],
                            ...
                            <tr class="hover:bg-[#FFF6F0]/50 transition">
                                ...
                                <td class="px-5 py-3.5">
                                    <span class="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-pink-50 text-brand whitespace-nowrap">
                                        {{ ucwords(str_replace('_', ' ', $r->reason ?? $r->return_type ?? '—')) }}
                                    </span>
                                </td>
...
<script>
    ...
    async function changeStatus(sel) {
        ...
        if (prev === next) return;
        sel.disabled = true;
        const res = await post('/admin/returns/' + sel.dataset.id + '/status', { status: next });
        sel.disabled = false;
        if (res.success) {
