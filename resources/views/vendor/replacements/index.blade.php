...
<body class="min-h-screen" style="background-color: #FFF6F0;">
    <div class="flex min-h-screen">
        ...
        <x-vendor.app-header title="Replace Orders" subtitle="{{ $stats['total'] }} requests · {{ $stats['pending'] }} pending" />
        <main class="flex-1 p-4 sm:p-6 pb-28 md:pb-8 overflow-y-auto page-enter">
            ...
            <!-- Mobile replacement cards (rendered by JS from the table rows below) -->
            <div id="replacementsMobileList" class="md:hidden space-y-3"></div>
    ...
    <script>
        ...
        async function filterReplacements(status) {
            ...
            try {
                ...
                if (typeof renderMobileReplacementCards === 'function') renderMobileReplacementCards();
            }
        }
    ...
    <script>
        (function () {
            ...
            window.renderMobileReplacementCards = function () {
                const list = document.getElementById('replacementsMobileList');
                if (!list) return;

                const rows = document.querySelectorAll('#replacementsTableBody tr[data-replacement-id]');
                if (!rows.length) {
                    list.innerHTML = `
                        <div class="app-card p-10 text-center">
                            ...
                            <p class="text-gray-600 text-sm font-semibold">No replacement requests found</p>
                            <p class="text-gray-400 text-xs mt-1">When customers request replacements, they'll appear here.</p>
                        </div>`;
                    return;
                }
                ...
                let html = '';
                rows.forEach(row => {
                    const id = row.getAttribute('data-replacement-id');
                    const code = row.cells[0].textContent.trim();
                    const customer = (row.cells[1].querySelector('.font-medium')?.textContent || row.cells[1].textContent).trim().replace(/\s+/g, ' ');
                    ...
                    const step = (row.cells[4].textContent || '').trim().replace(/\s+/g, ' ');
                    const badgeEl = row.cells[5].querySelector('.status-badge');
                    const status = (badgeEl && badgeEl.getAttribute('data-current-status'))
                    ...
                    html += `
                    <div class="m-replacement-card app-card p-4">
                        <div class="flex items-center justify-between gap-2 mb-3">
                            ...
                            <span class="badge ${getStatusColor(status)} text-white cursor-pointer status-badge flex-shrink-0"
                                  data-replacement-id="${esc(id)}" data-current-status="${esc(status)}"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="text-[11px] text-gray-400 mt-0.5 truncate">${esc(customer)}${customer && reqDate ? ' &middot; ' : ''}${esc(reqDate)}</p>
                            </div>
                        </div>
                        <div class="mt-3 grid grid-cols-2 gap-2">
                            ...
                            <div class="brand-gradient-soft rounded-xl px-3 py-2">
                                <p class="text-[10px] text-gray-500 font-medium">Current Step</p>
                                <p class="text-[11px] font-semibold text-gray-800 truncate">${esc(step)}</p>
                            </div>
                        </div>
                        <div class="mt-3 pt-3 border-t border-[#E85D85]/10 flex gap-2">
                            <button onclick="viewReplacement(${parseInt(id, 10)})"
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if (search) {
                    search.addEventListener('keyup', function () {
                        ...
                        document.querySelectorAll('#replacementsMobileList .m-replacement-card').forEach(card => {
